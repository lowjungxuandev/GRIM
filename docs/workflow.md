...
- `POST /api/v1/import` keeps the HTTP connection open until **Cloudinary**, both OpenRouter model steps, and the **Realtime Database** write complete (then receiver **FCM** on success). Progress is indicated by SSE `data:` lines, not by returning before work finishes.

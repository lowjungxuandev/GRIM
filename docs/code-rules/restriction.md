...
- **Do not** make **`test/app.test.ts`** (or other **`createApp`** suites) depend on real **`NVAPI_KEY`**, Firebase, or Cloudinary unless that is an explicit, documented exception—**`unit-test-rules.md`** splits app doubles vs lib integration.
